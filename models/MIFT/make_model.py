...
from .backbones.vit_pytorch import multiscale
import torch.nn.functional as F



class Gem_heat(nn.Module):
    def __init__(self, dim = 512, p=3, eps=1e-6):
...
class build_transformer(nn.Module):
    ...
    def forward(self, x):
        ...
        tranformer_feature = self.classifier1(x)
        if self.block==1:
