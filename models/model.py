...
from .MIFT import make_transformer_model
...
import torch
def make_model(opt):
    model_path='D:\MIFT\pretrain_model\Multi-scale-former.pth'
    if opt.views == 2:
        model = two_view_net(opt.nclasses, block=opt.block,return_f=opt.triplet_loss)
        model.model_1.transformer.load_state_dict(torch.load(model_path))

    elif opt.views == 3:
        model = three_view_net(opt.nclasses, share_weight=opt.share,block=opt.block,return_f=opt.triplet_loss)
        model.model_1.transformer.load_state_dict(torch.load(model_path))
        model.model_2.transformer.load_state_dict(torch.load(model_path))
